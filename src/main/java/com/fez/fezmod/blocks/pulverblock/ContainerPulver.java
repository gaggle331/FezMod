...
public class ContainerPulver extends Container{
	// Stores a reference to the tile entity instance for later use
		private TileEntityPulverBlock tileEntityPulverBlock;
		...
		// slot index is the unique index for all slots in this container i.e. 0 - 35 for invPlayer then 36 - 49 for tileEntityPulverBlock
		...
		// slot number is the slot number within each component; i.e. invPlayer slots 0 - 35, and tileEntityPulverBlock slots 0 - 14
		...
		public ContainerPulver(InventoryPlayer invPlayer, TileEntityPulverBlock tileEntityPulverBlock) {
			this.tileEntityPulverBlock = tileEntityPulverBlock;
			...
			for (int x = 0; x < FUEL_SLOTS_COUNT; x++) {
				...
				addSlotToContainer(new SlotFuel(tileEntityPulverBlock, slotNumber, FUEL_SLOTS_XPOS + SLOT_X_SPACING * x, FUEL_SLOTS_YPOS));
			}
			...
			for (int y = 0; y < INPUT_SLOTS_COUNT; y++) {
				...
				addSlotToContainer(new SlotSmeltableInput(tileEntityPulverBlock, slotNumber, INPUT_SLOTS_XPOS, INPUT_SLOTS_YPOS+ SLOT_Y_SPACING * y));
			}
			...
			for (int y = 0; y < OUTPUT_SLOTS_COUNT; y++) {
				...
				addSlotToContainer(new SlotOutput(tileEntityPulverBlock, slotNumber, OUTPUT_SLOTS_XPOS, OUTPUT_SLOTS_YPOS + SLOT_Y_SPACING * y));
			}
		}
		...
		{
			return tileEntityPulverBlock.isUseableByPlayer(player);
		}
		...
		public ItemStack transferStackInSlot(EntityPlayer player, int sourceSlotIndex)
		{
			...
			if (sourceSlotIndex >= VANILLA_FIRST_SLOT_INDEX && sourceSlotIndex < VANILLA_FIRST_SLOT_INDEX + VANILLA_SLOT_COUNT) {
				...
				if (tileEntityPulverBlock.getPulverResult(sourceStack) != null){
					if (!mergeItemStack(sourceStack, FIRST_INPUT_SLOT_INDEX, FIRST_INPUT_SLOT_INDEX + INPUT_SLOTS_COUNT, false)){
						return null;
					}
				}	else if (tileEntityPulverBlock.getItemBurnTime(sourceStack) > 0) {
		...
		public void detectAndSendChanges() {
			...
			boolean fieldHasChanged [] = new boolean[tileEntityPulverBlock.getFieldCount()];
			if (cachedFields == null) {
				cachedFields = new int[tileEntityPulverBlock.getFieldCount()];
				allFieldsHaveChanged = true;
			}
			for (int i = 0; i < cachedFields.length; ++i) {
				if (allFieldsHaveChanged || cachedFields[i] != tileEntityPulverBlock.getField(i)) {
					cachedFields[i] = tileEntityPulverBlock.getField(i);
			...
			for (int i = 0; i < this.crafters.size(); ++i) {
				...
				for (int fieldID = 0; fieldID < tileEntityPulverBlock.getFieldCount(); ++fieldID) {
		...
		public void updateProgressBar(int id, int data) {
			tileEntityPulverBlock.setField(id, data);
		}
		...
		public class SlotFuel extends Slot {
			...
			public boolean isItemValid(ItemStack stack) {
				return tileEntityPulverBlock.isItemValidForFuelSlot(stack);
			}
		}
		...
		public class SlotSmeltableInput extends Slot {
			...
			@Override
			public boolean isItemValid(ItemStack stack) {
				return tileEntityPulverBlock.isItemValidForInputSlot(stack);
			}
		}
		...
		public class SlotOutput extends Slot {
			...
			@Override
			public boolean isItemValid(ItemStack stack) {
				return tileEntityPulverBlock.isItemValidForOutputSlot(stack);
			}
		}
	}
