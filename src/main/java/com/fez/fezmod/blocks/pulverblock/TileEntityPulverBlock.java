...
public class TileEntityPulverBlock extends TileEntity implements IInventory, ITickable{
	// Create and initialize the itemStacks variable that will store store the itemStacks
		...
		private static int COOK_TIME_FOR_COMPLETION = 200;  // vanilla value is 200 = 10 seconds
		...
		// - see if any of the items have finished pulving
		// It runs both on the server and the client.

		@Override
		public void update() {
			// If there is nothing to pulv or there is no room in the output, reset cookTime and return
			if (canPulv()) {
				...
				// If cookTime has reached maxCookTime pulv the item and reset cookTime
		...
		/**
		 * Check if any of the input items are pulvable and there is sufficient space in the output slots
		 * @return true if pulving is possible
		 */
		private boolean canPulv() {return pulvItem(false);}

		/**
		 * Pulv an input item into an output slot, if possible
		 */
		private void pulvItem() {pulvItem(true);}

		/**
		 * checks that there is an item to be pulved in one of the input slots and that there is room for the result in the output slots
		 * If desired, performs the pulv
		 * @param performPulv if true, perform the pulv.  if false, check whether pulving is possible, but don't change the inventory
		 * @return false if no items can be pulved, true otherwise
		 */
		private boolean pulvItem(boolean performPulv)
		{
			Integer firstSuitableInputSlot = null;
			Integer firstSuitableOutputSlot = null;
			ItemStack result = null;

			// finds the first input slot which is pulvable and whose result fits into an output slot (stacking if possible)
			for (int inputSlot = FIRST_INPUT_SLOT; inputSlot < FIRST_INPUT_SLOT + INPUT_SLOTS_COUNT; inputSlot++)	{
				if (itemStacks[inputSlot] != null) {
					result = getPulverResult(itemStacks[inputSlot]);
					COOK_TIME_FOR_COMPLETION = getPulverTime(itemStacks[inputSlot]);
	  			if (result != null) {
		...
		// returns the pulving result for the given stack. Returns null if the given stack can not be pulved
		public static ItemStack getPulverResult(ItemStack stack) { return PulverRecipes.instance().getPulverResult(stack); }
		
		public static int getPulverTime(ItemStack stack) {
			return PulverRecipes.instance().getPulverTime(stack);
		}
...
//			// Save the stored item stacks
		...
		public String getName() {
			return "container.PulverBlock.name";
		}
		...
		public int getField(int id) {
			...
			System.err.println("Invalid field ID in TileInventoryPulving.getField:" + id);
			return 0;
		}
		...
		public void setField(int id, int value)
		{
			if (id == COOK_FIELD_ID) {
				...
				System.err.println("Invalid field ID in TileInventoryPulving.setField:" + id);
			}
		}
