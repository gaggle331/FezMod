...
public class TileEntityFurnaceBlock extends TileEntity implements IInventory, ITickable{
	// Create and initialize the itemStacks variable that will store store the itemStacks
		...
		private boolean smeltItem(boolean performSmelt)
		{
			...
			for (int inputSlot = FIRST_INPUT_SLOT; inputSlot < FIRST_INPUT_SLOT + INPUT_SLOTS_COUNT; inputSlot++)	{
				if (itemStacks[inputSlot] != null) {
					result = getFurnaceResult(itemStacks[inputSlot]);
					COOK_TIME_FOR_COMPLETION = getFurnaceCookTime(itemStacks[inputSlot]);
					//System.out.println(COOK_TIME_FOR_COMPLETION);
	  			if (result != null) {
		...
		public static ItemStack getFurnaceResult(ItemStack stack) { 
			return ModFurnaceRecipes.instance().getFurnaceResult(stack); 
			}
		public static int getFurnaceCookTime(ItemStack stack) {
			return ModFurnaceRecipes.instance().getFurnaceCookTime(stack);
		}
...
//			// Save the stored item stacks
		...
		public String getName() {
			return "container.FurnaceBlock.name";
		}
